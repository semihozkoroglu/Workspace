function [J,N,K,Szig,Srun,Sinrun,Sinzig] = compress(I)

    f = double(imread(I));
    N = f;
    for i = 1:3
        [J{i},N(:,:,i),K{i},Szig{i},Srun{i},Sinrun{i},Sinzig{i}] = isleme(f(:,:,i));
    end
    
    N = uint8(N);
    
function [J,N,K,Szig,Srun,Sinrun,Sinzig] = isleme(f)
